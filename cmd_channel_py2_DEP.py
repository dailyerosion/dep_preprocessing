...
def FillDEM(inDEM):
    # Fill the DEM and copy to an uncompressed version
    arcpy.AddMessage("DEM processing...")
    outFill = Fill(inDEM)
...
##        if useEOWH8 == "true": 
##               eow12FDir = "in_memory\tempFDir"
##               eow12FDir = ExtractByMask(EOWFDirHUC8, arcFDir)
##
##               arcpy.Mosaic_management([eow12FDir], arcFDir)
##               arcpy.Delete_management(eow12FDir)
        ...
        # as a fix to the no channel problem, try using Taudem flow dir
##        callstr = 'mpiexec -n 7 D8Flowdir ' + \
##                  '-p "' +  os.path.join(ProcDir, 'demp.tif') + '" ' + \
##                  '-sd8 "' +  os.path.join(ProcDir, 'demsd8.tif') + '" ' + \
##                  '-fel "' + filledDEM.getOutput(0) + '"'
##        os.system(callstr)
...
def extractPoutPts(fileGDB, huc12, WSBndsrc):
    ...
    ws_bnd_shp = arcpy.FeatureToLine_management(ws_bndy_lyr, "WS_bnd.shp")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        cleanup = False
        parameters = ['C:\\Python27\\ArcGISx6410.3\\pythonw.exe', 'C:\\DEP\\Scripts\\basics\\cmd_gen1_pkdoug_chnl_tau_bkg5.py', '070300040804', '2022', '26915', 'mean18', '3']
    ...
    try:
        inDEM = sys.argv[1]#paths['cElevFile']
        ProcDir = sys.argv[2]#paths['cutProcDir']
        statGDB = sys.argv[3]#paths['mwHuc12s5070'] + '_Status_' + interpType
        pdCatch = sys.argv[4]#paths['pdCatch']
        pdChnl = sys.argv[5]#paths['pdChnl']#
        wShed = sys.argv[6]#paths['wShed']
        fileGDB = os.path.dirname(pdCatch)#sys.argv[3]
        huc12 = os.path.splitext(os.path.basename(inDEM))[0][-12:]
        ...
        log.info('Peukering on ' + huc12 + " at " + time.asctime())
        ...
        extractPoutPts(fileGDB, huc12, WSBndsrc)

        chThresh = mkPDougStrm(ProcDir)

        mkWSheds(ProcDir, sgdb, huc12, WSBndsrc, log, pdCatch, wShed)
