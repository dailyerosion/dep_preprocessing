...
def selectFrAndWsDpPts(gdbTable, dnPts, frFld, sfx, inm = 'in_memory\\'):
    dnFrWsList = []
...
##            gcSel4step = buildSelection(sublist, in_field)
            ...
            for index, item in enumerate(sublist):
                if index == 0:
                    sel = frFld + ' = ' +str(item[0]) + ' AND ws_lvl = ' + str(item[1])
                else:
                    sel += ' OR ' + frFld + ' = ' +str(item[0]) + ' AND ws_lvl = ' + str(item[1])
...
##def setWsSearchDist(inFC, wsSearchDistFld, minFrDistFld, frFld, minElFld, ofElFld, srchMult, crestMin, maxWsMdn, maxWsMdnFld):
